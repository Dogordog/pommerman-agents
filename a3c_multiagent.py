"""
Adaptation of a3c.py to experiment training with all players
"""
...
def sample_dist(dists):
	actions = []
	for dist in dists:
		actions.append(np.random.choice(np.arange(len(dist)), p=dist))
	return actions
		...
		with sess.as_default(), sess.graph.as_default():				 
			while not coord.should_stop():
				...
				for i, agent in enumerate(self.agent_list):
					state = process_observation(obs[i], agent)
					states.append(state)
				...
				while not episode_end:
					...
					if FLAGS.render:
						self.env.render()
					bot_actions = self.env.act(obs)
					obs, r, episode_end, info = self.env.step(actions)
					wood_wall = np.sum(obs[0]['board'] == 2)
					
					r = []
					for i, agent in enumerate(self.agent_list):
						if [10, 11, 12, 13][i] in obs[0]['alive']:
							if agent.prev_enemies != (len(obs[i]['alive']) - 1) and 10 in obs[i]['alive']:
								r.append(10.)
							elif agent.prev_ammo == (agent.ammo - 1) and prev_wood_wall != wood_wall: # need to differentiate between ammo decreasing, ammo returning to me and ammo increasing due to powerup
								r.append(1.)
							else:
								r.append(0.)
						else:
					...
					for i, agent in enumerate(self.agent_list):
						if [10, 11, 12, 13][i] in obs[0]['alive']:
							episode_buffer[i].append([np.expand_dims(s[i], axis=0), actions[i], r[i], np.expand_dims(s1[i], axis=0), episode_end, v[i,0]])
							episode_values[i].append(v[i, 0])
							episode_reward[i] += r[i]
					s = s1				 
					...
					for i, agent in enumerate(self.agent_list):
						if len(episode_buffer[i]) == 30 and not episode_end and episode_step_count != max_episode_length - 1:
							# Since we don't know what the true final return is, we "bootstrap" from our current value estimation.
							v1 = sess.run(self.local_AC.value, 
								feed_dict={self.local_AC.inputs: np.expand_dims(states[i], axis=0)})[0, 0]
							v_l,p_l,e_l,g_n,v_n = self.train(episode_buffer[i],sess,gamma,v1)
							episode_buffer[i] = []
							sess.run(self.update_local_ops)
					if episode_end:
						break
				...
				global _max_score, _running_avg_scores
				if _max_score < max(episode_reward):
					_max_score = max(episode_reward)
				for i, agent in enumerate(self.agent_list):
					_running_avg_scores[i] = (2.0 / 101) * (episode_reward[i] - _running_avg_scores[i]) + _running_avg_scores[i]

				if episode_count % 10 == 0:
					print("{} Step #{} Episode #{} Reward: {}".format(self.name, total_steps, episode_count, episode_reward))
					print("Total Steps: {}\tTotal Episodes: {}\tMax Score: {}\tAvg Score: {:.3f}/{:.3f}/{:.3f}/{:.3f}".format(sess.run(self.global_steps), sess.run(self.global_episodes), _max_score, _running_avg_scores[0], _running_avg_scores[1], _running_avg_scores[2], _running_avg_scores[3]))

				# Update the network using the episode buffer at the end of the episode.
				for i, agent in enumerate(self.agent_list):
					if len(episode_buffer[i]) != 0:
						v_l,p_l,e_l,g_n,v_n = self.train(episode_buffer[i],sess,gamma,0.0)
